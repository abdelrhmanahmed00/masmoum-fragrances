...
import {
  ...
  type BottleColorImageActionState,
...
// lib/admin/private-label-images.ts / lib/admin/perfume-gender-images.ts
// -- see either file's own comment for the full ordering reasoning
// (upload new, update DB, only THEN delete old), reused unchanged here
// for a different table/bucket.

const BUCKET = "bottle-color-images";

// Same limits as every other image-upload bucket in this project (0036
// migration's own real config).
...
function isValidSlot(value: string): value is BottleColorSlot {
  return (BOTTLE_COLOR_SLOTS as readonly string[]).includes(value);
}
...
/** All 5 slots, including any not-yet-uploaded (storage_path: null) --
 *  the admin page always shows every slot, never a partial list, same
 *  reasoning as getPrivateLabelImages'/getPerfumeGenderImages' own
 *  comment. */
export async function getBottleColorImages(
  supabase: SupabaseClient
): Promise<AdminBottleColorImageRow[]> {
  const { data, error } = await supabase
    .from("bottle_color_images")
    .select("slot, storage_path");

  return error || !data ? [] : (data as AdminBottleColorImageRow[]);
}
 ...
 */
export async function updateBottleColorImage(
  supabase: SupabaseClient,
  slot: string,
  formData: FormData
