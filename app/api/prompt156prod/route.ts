...
// TEMPORARY -- Prompt 156 gender-fix route, deleted immediately after
// use. Deployed to and invoked against PRODUCTION. Reuses the real
// updateProduct() -- its validate() does a FULL FormData replace, so
// every product's CURRENT full row is fetched first and reconstructed
// into a complete FormData with every field preserved except gender,
// same "fetch full row, rebuild full FormData" technique as Prompts
// 145/151.
const WOMEN_SLUGS = ["094", "095", "096", "097", "098", "099"];
const MEN_SLUGS = [
  "006", "007", "008", "009", "010",
...
export async function POST(request: Request) {
  ...
  const allSlugs = [...WOMEN_SLUGS, ...MEN_SLUGS];
  const { data: products, error: fetchError } = await supabase
    .from("products")
    .select("*")
    .in("slug", allSlugs);

  if (fetchError || !products || products.length !== allSlugs.length) {
    return NextResponse.json({
      status: "error",
      step: "fetch-products",
      message: fetchError?.message ?? `expected ${allSlugs.length}, got ${products?.length}`,
    });
  }

  const results: { slug: string; id: string; newGender: string; status: string; message?: string }[] = [];

  for (const p of products) {
    const newGender = WOMEN_SLUGS.includes(p.slug) ? "women" : "men";
    ...
    form.set("gender", newGender); // <-- the only real change
