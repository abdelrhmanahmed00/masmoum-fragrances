...
import { getAllActiveProducts, getBrandsWithActiveProducts } from "@/lib/catalog";
...
export async function GET() {
  // Mode B: reproduce page.tsx's exact call shape -- Promise.all
  // preamble with getBrandsWithActiveProducts() running alongside, then
  // getAllActiveProducts({ brandId: null }) (not the zero-arg call) --
  // to test whether that specific shape reproduces the staleness seen
  // on the real page but NOT on this route's simpler zero-arg call.
  const [, products] = await Promise.all([
    getBrandsWithActiveProducts(),
    getAllActiveProducts({ brandId: null }),
  ]);
