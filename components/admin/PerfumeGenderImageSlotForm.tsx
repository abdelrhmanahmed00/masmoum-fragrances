...
import { updatePerfumeGenderImageAction } from "@/app/admin/(dashboard)/perfume-gender/actions";
import { compressImage } from "@/lib/image-compression";
import { getPublicStorageUrl } from "@/lib/supabase/storage";
import {
  PERFUME_GENDER_IMAGE_ACTION_INITIAL_STATE,
  type PerfumeGenderSlot,
} from "@/types/admin-perfume-gender";

// Same 5MB/JPEG-PNG-WEBP limits as the perfume-gender-images Storage
// bucket's real config (0034 migration) and lib/admin/perfume-gender-images.ts's
// own server-side re-check -- client-side only for immediate UX feedback.
const MAX_FILE_SIZE_BYTES = 5 * 1024 * 1024;
const ACCEPTED_TYPES = ["image/jpeg", "image/png", "image/webp"];

/**
 * One of the 3 fixed-slot upload widgets on the Perfumes Gender Tiles
 * admin page -- byte-for-byte the same shape/compression wiring as
 * PrivateLabelImageSlotForm.tsx (Prompt 92/127), reused for a different
 * table/bucket/action rather than generalizing that component: this
 * project's own established precedent (HeroSlideForm, ProductImagesSection,
 * PrivateLabelImageSlotForm are each their own separate component despite
 * strong conceptual overlap) is to keep each feature's admin form as its
 * own small, directly-readable file rather than force a shared
 * abstraction across genuinely different tables/actions.
 */
export default function PerfumeGenderImageSlotForm({
  slot,
  label,
  currentStoragePath,
}: {
  ...
  const action = updatePerfumeGenderImageAction.bind(null, slot);
  ...
  // Same reset+refresh pattern as PrivateLabelImageSlotForm.tsx: clear
  // the file input and re-fetch this Server Component's own data so the
  ...
  return (
    <div className="flex items-center gap-4 rounded-card border border-brand-border p-4">
      {/* Current image preview, or a plain placeholder square if this
          ...
          never a broken <img>" convention as
          PrivateLabelImageSlotForm.tsx's own. */}
      <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-btn bg-brand-surface">
        {currentStoragePath ? (
          ...
          // this project.
          <img
            src={getPublicStorageUrl("perfume-gender-images", currentStoragePath)}
            alt=""
